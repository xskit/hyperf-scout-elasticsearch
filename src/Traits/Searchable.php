...
use Exception;
use XsKit\ScoutElasticSearch\Builders\FilterBuilder;
use XsKit\ScoutElasticSearch\Builders\SearchBuilder;
use XsKit\ScoutElasticSearch\ElasticEngine;
use XsKit\ScoutElasticSearch\IndexConfigurator;
use XsKit\ScoutElasticSearch\SearchRule;
...
trait Searchable
{
    ...
    public function queueMakeSearchable($models)
    {
        ...
        try {
            ...
            printf('[INFO] latest [%s] records have been imported' . PHP_EOL, $models->last()->getScoutKey());
        } catch (\Throwable $e) {
            printf('[ERROR] Elastic import： message:%s ,file:%s(%s Line),code: %s' . PHP_EOL, $e->getMessage(), $e->getFile(), $e->getLine(), $e->getCode());
        }
    }
    ...
    {
        $softDelete = static::usesSoftDelete() && config('scout_elasticsearch.soft_delete', false);
        if ($query === '*') {
    ...
    {
        return config('scout_elasticsearch.prefix') . $this->getTable();
    }
    ...
    public function getMapping()
    {
        ...
        if ($this::usesSoftDelete() && config('scout_elasticsearch.soft_delete', false)) {
    ...
    public function searchableUsing()
    {
        ...
        $indexerClass = '\\XsKit\\ScoutElasticSearch\\Indexers\\' . ucfirst($indexerType) . 'Indexer';
